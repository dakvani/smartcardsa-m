...
export function AddToWalletButtons({ profile, links, publicUrl }: AddToWalletButtonsProps) {
  ...
  return (
    <div className="sm:hidden mt-2.5 flex w-full items-center justify-center gap-2">
      <button type="button" onClick={() => handleAdd("apple")} disabled={busy !== null} className={base}>
        {busy === "apple" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Apple className="h-3.5 w-3.5" />}
        Apple Wallet
      </button>
      <button type="button" onClick={() => handleAdd("google")} disabled={busy !== null} className={base}>
