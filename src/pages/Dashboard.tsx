            ...
            <div className="grid grid-cols-4 gap-1.5 sm:gap-2.5">
              {[
                ...
                <motion.div
                  key={stat.label}
                  whileHover={{ y: -2 }}
                  className={`relative overflow-hidden rounded-md border border-border/60 bg-gradient-to-br ${stat.bg} bg-background/40 backdrop-blur-sm px-2 py-1.5`}
                >
                  <div className="flex items-center gap-1.5">
                    <stat.icon className={`w-3.5 h-3.5 shrink-0 ${stat.color}`} />
                    <div className="min-w-0 flex items-baseline gap-1.5">
                      <p className="text-sm font-bold tabular-nums leading-none">{stat.value.toLocaleString()}</p>
                      <p className="text-[9px] uppercase tracking-wider text-muted-foreground font-medium truncate">{stat.label}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
